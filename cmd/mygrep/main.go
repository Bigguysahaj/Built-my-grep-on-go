...
func handleBackReference(state *MatchState) bool {
	refNum := int(state.pattern[state.patternIndex+1] - '0')
	if refNum >= len(state.parenExpressions) {
		return false
	}
	refContent := state.parenExpressions[refNum]
	if !strings.HasPrefix(string(state.line[state.lineIndex:]), refContent) {
		return false
	}
	state.lineIndex += len(refContent)
	state.patternIndex++  // Move past the backreference
	return true
}
...
func handleParentheses(state *MatchState) bool {
	...
	return handleSimpleParentheses(state)
}

func handleSimpleParentheses(state *MatchState) bool {
	closeParen := bytes.IndexByte([]byte(state.pattern[state.patternIndex:]), ')')
	if closeParen == -1 {
		return false
	}
	...
	// Capture the group
	capturedContent := string(state.line[state.lineIndex:state.lineIndex+len(expression)])
	state.parenExpressions = append(state.parenExpressions, capturedContent)
	
	if !strings.HasPrefix(string(state.line[state.lineIndex:]), expression) {
		return false
	}
	
	state.lineIndex += len(expression)
	state.patternIndex += closeParen + 1
	return true
}
